...
export const login = async (req: Request, res: Response<ApiResponseBody>) => {
  ...
  try {
    ...
    if (token) {
      res.cookie('token', token, { maxAge: 3600000 * 24, domain: process.env.DOMAIN, path: '/', httpOnly: true, sameSite: "none", secure: process.env.NODE_ENV === 'production', signed: true})
      res.status(200).json({
        code: 200,
        status: 'success',
